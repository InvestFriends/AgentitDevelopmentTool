...
const navGroups = [
  {
    label: 'Dashboardy',
    items: [
      { to: '/dashboard/management', icon: LayoutDashboard, label: 'Management' },
      { to: '/dashboard/team', icon: Users, label: 'Týmový' },
      { to: '/dashboard/agent', icon: Bot, label: 'Agenti' },
    ],
  },
  {
    label: 'Správa',
  ...
  return (
    <aside className="w-56 min-h-screen bg-slate-900 text-slate-100 flex flex-col">
      ...
      <nav className="flex-1 px-2 py-4 space-y-4">
        {navGroups.map(group => (
          <div key={group.label}>
            <div className="px-3 mb-1 text-xs font-semibold text-slate-500 uppercase tracking-wider">{group.label}</div>
