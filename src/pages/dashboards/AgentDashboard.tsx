...
async function fetchAgentWorkload() {
  const [agentsRes, ticketsRes] = await Promise.all([
    ...
    supabase.from('tickets').select('assigned_agent_id, status').not('status', 'in', '("CLOSED","CANCELLED","REJECTED","RELEASED")'),
  ])
...
export default function AgentDashboard() {
  ...
  const workload = agents.map((a, i) => ({
    ...
    fullName: a.name,
    count: tickets.filter(t => t.assigned_agent_id === a.id).length,
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border rounded-lg p-4">
          ...
          <div className="text-3xl font-bold text-amber-500 mt-1">
            {tickets.filter(t => !t.assigned_agent_id).length}
          </div>
        </div>
      </div>
